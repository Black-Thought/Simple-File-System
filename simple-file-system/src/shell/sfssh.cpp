...
void do_write(Disk &disk, FileSystem &fs, int argc, char *inode_str, char *text) {
    if (argc < 3) {
        fprintf(stderr, "Usage: write <inode> <text>\n");
        return;
    }

    if (!disk.mounted()) {
        fprintf(stderr, "No mounted filesystem.\n");
        return;
    }

    size_t inumber = atoi(inode_str);
    if (strlen(text) == 0) {
        fprintf(stderr, "No text provided.\n");
        return;
    }

    ssize_t result = fs.write(inumber, text, strlen(text), 0);

    if (result < 0) {
        fprintf(stderr, "Write failed.\n");
    } else {
        printf("Wrote %zd bytes to inode %zu.\n", result, inumber);
    }
}
#include <vector>   // at top of file if not already

void do_read(Disk &disk, FileSystem &fs, int /*args*/, char *inode_str, char * /*unused*/) {
    if (!disk.mounted()) {
        fprintf(stderr, "No mounted filesystem.\n");
        return;
    }

    if (!inode_str || strlen(inode_str) == 0) {
        fprintf(stderr, "Usage: read <inode>\n");
        return;
    }

    size_t inumber = (size_t)atoi(inode_str);

    // Get size first
    ssize_t sz = fs.stat(inumber);
    if (sz < 0) {
        fprintf(stderr, "read: bad inode or stat failed\n");
        return;
    }
    if (sz == 0) {
        // Empty file — print nothing but succeed
        printf("\n");
        return;
    }
    ...
    ssize_t n = fs.read(inumber, buf.data(), (size_t)sz, 0);
    if (n < 0) {
...
int main(int argc, char *argv[]) {
	...
	if (streq(cmd, "debug")) {
	    ...
	    do_copyin(disk, fs, args, arg1, arg2);
            ...
            do_write(disk, fs, args, arg1, arg2);
        } else if (streq(cmd, "read")) {
            do_read(disk, fs, args, arg1, arg2);
